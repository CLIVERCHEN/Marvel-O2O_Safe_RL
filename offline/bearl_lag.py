...
from config.bearl_config import BEARL_DEFAULT_CONFIG, BEARLTrainConfig
...
def train(args: BEARLTrainConfig):
    ...
    results_dir = f"../offline_dataset_bearl/{args.task}"
    ...
    for step in trange(args.update_steps, desc="Training"):
        ...
        # evaluation
        if (step + 1) % 1000 == 0 or step == args.update_steps - 1:
            ...
            torch.save(model.actor.state_dict(), os.path.join(save_dir, f'{args.task}_bearl_actor.pth'))
            torch.save(model.critic.state_dict(), os.path.join(save_dir, f'{args.task}_bearl_critic.pth'))
            torch.save(model.cost_critic.state_dict(),
                       os.path.join(save_dir, f'{args.task}_bearl_cost_critic.pth'))

            torch.save(model.actor_optim.state_dict(),
                       os.path.join(save_dir, f'{args.task}_bearl_actor_optim.pth'))
            torch.save(model.critic_optim.state_dict(),
                       os.path.join(save_dir, f'{args.task}_bearl_critic_optim.pth'))
            torch.save(model.cost_critic_optim.state_dict(),
                       os.path.join(save_dir, f'{args.task}_bearl_cost_critic_optim.pth'))
